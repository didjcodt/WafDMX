...
static void animate_0() {
    int bn = 31;
    for (int led = 0; led < CONFIG_LEDS_COUNT; led++) {
        apa102_set_pixel(led, bn, led_dmx_buffer[3 * led],
                         led_dmx_buffer[3 * led + 1],
                         led_dmx_buffer[3 * led + 2]);
    }
    apa102_flush();
}

static void (*animate[])(void) = {animate_0, animate_1, animate_2, animate_3,
                                  animate_4};

static enum led_animation_name current_animation = ANIMATE_0;
